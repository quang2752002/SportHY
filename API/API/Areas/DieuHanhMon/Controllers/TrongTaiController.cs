...
{
    public class TrongTaiController : BaseDieuHanhMonController
    {
        public TrongTaiController(IDieuHanhMonService dieuHanhService, UserManager<ApplicationUser> userManager)
        ...
        public async Task<IActionResult> Index(int? giaiDauId, int? danhMucId, int? monTheThaoId)
        {
            ...
            {
                return View(new List<CoordinatorRefereeItemDto>());
            }
            ...
            if (currentAssignment == null) return View(new List<CoordinatorRefereeItemDto>());

            ViewBag.SportList = currentAssignment.MonTheThaos;

            var list = await _dieuHanhService.GetMonRefereesAsync(selectedGiaiDauId.Value, selectedDanhMucId.Value, monTheThaoId, assignments);
