        ...
        public async Task<IActionResult> Save([FromBody] CreateUpdateVanDongVienDto dto, [FromQuery] int? id = null)
        {
            ...
            try
            {
                if (id.HasValue && id.Value > 0)
                {
                    ...
                    return Json(new { success = true, message = "Cập nhật vận động viên thành công!", data = updated });
                }
                else
                {
                    var created = await _vanDongVienService.CreateAsync(dto, username);
                    return Json(new { success = true, message = "Thêm mới vận động viên thành công!", data = created });
                }
            }
            catch (Exception ex)
            {
