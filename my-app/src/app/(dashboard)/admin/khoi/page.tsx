...
import { khoiService } from '@/services';
import { Khoi, CreateUpdateKhoi } from '@/types';
import { PaginationComponent } from '@/components/common/PaginationComponent';
import { useAuth } from '@/context/AuthContext';
import { Permissions } from '@/constants/permissions';

export default function AdminKhoiPage() {
  const { hasPermission } = useAuth();
  const canCreate = hasPermission(Permissions.Khoi.Create);
  const canEdit = hasPermission(Permissions.Khoi.Edit);
  const canDelete = hasPermission(Permissions.Khoi.Delete);
  ...
  const [formData, setFormData] = useState<CreateUpdateKhoi>({
    ma: '',
    ten: '',
    moTa: '',
    trangThai: true,
  ...
  const [deletingItem, setDeletingItem] = useState<Khoi | null>(null);

  // Fetch dữ liệu từ backend
  const loadKhois = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const data = await khoiService.getPaged({
        pageIndex,
        pageSize,
  ...
  const handleOpenEditModal = (item: Khoi) => {
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    if (!formData.ma.trim() || !formData.ten.trim()) {
      alert('Vui lòng nhập đầy đủ Mã khối và Tên khối!');
      return;
    }
    ...
    try {
      if (editingId) {
        await khoiService.update(editingId, formData);
      } else {
        await khoiService.create(formData);
      }
      setModalOpen(false);
      loadKhois();
    } catch (err: any) {
      console.error('Lỗi khi lưu khối:', err);
      alert(err?.response?.data?.message || err?.message || 'Có lỗi xảy ra khi lưu khối.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleOpenDeleteModal = (item: Khoi) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await khoiService.delete(deletingItem.id);
      setDeleteModalOpen(false);
      setDeletingItem(null);
      loadKhois();
    } catch (err: any) {
      console.error('Lỗi khi xóa khối:', err);
      alert(err?.response?.data?.message || err?.message || 'Không thể xóa khối này.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <Col lg="12">
          ...
          <CardBody className="p-4">
            ...
            <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 pb-3 mb-4 border-bottom">
              ...
              {canCreate && (
                <div>
                  ...
                  >
                    <i className="bi bi-plus-lg"></i> Thêm khối mới
                  </Button>
                </div>
              )}
            </div>
                  ...
                  <Col md={6} lg={5}>
                    <div className="input-group bg-white rounded-3 overflow-hidden border">
                      ...
                      <Input
                        ...
                        placeholder="Tìm kiếm theo mã, tên khối..."
                        value={keyword}
            ...
            {loading ? (
              <div className="text-center py-5">
                ...
                <p className="mt-2 text-muted small">Đang tải danh sách khối...</p>
              </div>
            ) : khois.length === 0 ? (
              <div className="text-center py-5 text-muted border rounded-3 bg-light">
                ...
                Chưa có khối nào được ghi nhận.
              </div>
            ) : (
        ...
        <Modal isOpen={modalOpen} toggle={() => setModalOpen(!modalOpen)} backdrop="static" centered>
          <ModalHeader toggle={() => setModalOpen(!modalOpen)}>
            ...
            {editingId ? 'Cập nhật Khối Tham Gia' : 'Thêm Khối Tham Gia Mới'}
          </ModalHeader>
          <Form onSubmit={handleSubmitForm}>
            <ModalBody>
              <FormGroup>
                <Label className="fw-semibold">
                  Mã Khối <span className="text-danger">*</span>
                </Label>
                <Input
                  type="text"
                  placeholder="VD: KHOI_TRUONG, KHOI_DN..."
                  value={formData.ma}
              ...
              <FormGroup>
                <Label className="fw-semibold">
                  Tên Khối <span className="text-danger">*</span>
                </Label>
                <Input
                  type="text"
                  placeholder="VD: Khối Trường Học & Sinh Viên..."
                  value={formData.ten}
              ...
              <FormGroup>
                ...
                <Input
                  type="textarea"
                  rows={3}
                  placeholder="Mô tả chi tiết khối tham gia, đối tượng áp dụng..."
                  value={formData.moTa}
              ...
              <FormGroup switch className="mt-3">
                ...
                />
                <Label check for="trangThaiSwitch" className="fw-semibold ms-2">
        ...
        <Modal isOpen={deleteModalOpen} toggle={() => setDeleteModalOpen(!deleteModalOpen)} centered size="sm">
          ...
          <ModalBody>
            Bạn có chắc chắn muốn xóa khối <strong>{deletingItem?.ten}</strong> ({deletingItem?.ma}) không?
            <p className="text-muted small mt-2 mb-0">Hành động này sẽ chuyển trạng thái khối sang đã xóa.</p>
          </ModalBody>
          <ModalFooter>
